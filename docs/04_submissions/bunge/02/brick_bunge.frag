...
{
    // -- MY COMMENT --
    // -- Normalize the pixel coordinates (0 - 1), so they work for any screen size.--
    ...
    // -- divide the screen into rows and columns of bricks (+ gaps) --
    float x = coord.x / brick_mortar_w;
    float y = coord.y / brick_mortar_h;
    // float x = coord.x;
    // float y = coord.y;

    // -- Shifts every 2nd row of bricks to the right by half a brick width. To recreate this brick wall pattern. --
    ...
    // -- This part is to figure out if the pixel is inside the brick or not --
    // -- bias + smoothstep to soften the edges between brick and mortar --
    ...
    // -- mix two colors. Mortar and Brick color based on how much we are inside the brick fill. --
    vec3 color = mix(vec3(0.2, 0.15, 0.15), vec3(0.5, 0.25, 0.2), getBias(w * h, 0.2));

    // -- use final color of the pixel. 1.0 is probably the alpha (not transparent)  --
