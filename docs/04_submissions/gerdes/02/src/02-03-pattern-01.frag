...
float oscillate(float time, float minValue, float maxValue, float speedDivisor) {
    ...
    float amplitude = (maxValue - minValue) * 0.5;
...
void main() {
    ...
    coord *= 8.0; // zoom factor

    vec2 id = floor(coord); // assign ids to cells
    float check = mod(id.x + id.y, 2.0); // assign 1.0 to every second (even id) cell

    vec3 color = vec3(0);
    // color += check;

    coord = fract(coord) - .5; // used to repeat the pattern based on the centered coordinate system
    vec2 p = abs(coord);

    if(check == 1.) p = p.yx; 

    float k = oscillate(u_time, .5, 1.0, 2.0);
    float a = k * 3.1415; // .5..1.0
    vec2 n = vec2(sin(a), cos(a));

    float d = dot(p - .48, n); // distance to line
    d = min(d, p.x);
    d = max(d, - p.y);
    ...
    color += smoothstep( .01, 0., d - .02); // draw line
