...
void main()
{
    //// Normalising the coordinates between 0 and 1
    ...
    //// We are scaling the coordinates from screen-size to the size of one tile

    float x = coord.x / brick_mortar_w;
    float y = coord.y / brick_mortar_h;

    //// y_index rounds down coordinates within the tile-size, then with a modula operation we check wheter the remainder is
    //// 0 or 1, if it is 0 we add 0.5 to the x coordinate, if it is 1 we leave it as it is
    ...
    //// This step makes the tiles repeat, instead of pixels outside the first tile getting stretched
    x -= floor(x);
    y -= y_index;
    ...
    //// This step fades the pixel colour in or out depending on how close the pixel is to the brick edges.
    //// It also enables smooth transitions at the boundaries between brick and mortar.

    float w = getBias(smoothstep(0.0, mortar_half_norm_w, x), 0.1) 
                - getBias(smoothstep(1.0 - mortar_half_norm_w, 1.0, x), 0.3);
    ...
    //// This step replaces the grey colour with a brownish one through the mix function.
    vec3 color = mix(vec3(0.2, 0.15, 0.15), vec3(0.5, 0.25, 0.2), getBias(w * h, 0.8));


    //// The color (complemented by the value of the alpha channel) gets applied to the output pixel.
