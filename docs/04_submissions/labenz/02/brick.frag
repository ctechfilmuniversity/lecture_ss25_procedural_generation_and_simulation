...
void main()
{
    ...
    float mortar_half_norm_w = MORTAR / brick_mortar_w;
    float mortar_half_norm_h = MORTAR / brick_mortar_h;

    // BRICK PATTERN
    // TODO: comment the following steps

    // we increase the size of x and y so we can get multiple bricks on the canvas
    float x = coord.x / brick_mortar_w ;
    float y = coord.y / brick_mortar_h;
    // float x = coord.x;
    // float y = coord.y;

    // we shift every second row of bricks by 0.5 of the brick width
    ...
    // x and y are now in the range of 0-1 by subtracting the integer part
    x -= floor(x);
    y -= y_index;

    // we calculate if its a brick or mortar
    // Simplified versions:
    // float w = step(mortar_half_norm_w, x);
    // float h = step(mortar_half_norm_h, y);
    // or
    ...
    // we mix the color of the brick and the mortar
    vec3 color = mix(vec3(0.2, 0.15, 0.15), vec3(0.5, 0.25, 0.2), w * h);

// we returning the final color
