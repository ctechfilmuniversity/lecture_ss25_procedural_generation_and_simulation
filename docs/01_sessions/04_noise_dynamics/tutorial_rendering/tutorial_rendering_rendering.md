...
If you wan't to use Path Tracing in your render, make sure it is enabled in your project by going to ``Edit > Project Settings`` and then checking these boxes:
...
Smoothes out jagged edges, improves motion blur and decreases noise froim ray tracing.
...
The more temporal samples the renderer takes, the higher the quality of your motion blur will be, but it significantly increases the render time as each frame is calculated multiple times, to interpolate for the final the frame.
...
It **improves your overall render quality by slicing each frame up into a set amount of tiles** and rendering each one seperately. 
